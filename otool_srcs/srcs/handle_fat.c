...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/17 21:12:16 by clanier           #+#    #+#             */
/*   Updated: 2017/12/17 21:25:46 by clanier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		handle_fat_64(t_file file, uint32_t nfat_arch)
{
	...
	if (nfat_arch < 1 || check_size(&file,
	sizeof(struct fat_arch_64) * nfat_arch, F_OFFSET) < 0)
		return (-1);
	i = nfat_arch;
...
int		handle_fat(t_file file, uint32_t nfat_arch)
{
	...
	if (nfat_arch < 1 || check_size(&file,
	sizeof(struct fat_arch) * nfat_arch, F_OFFSET) < 0)
		return (-1);
	i = nfat_arch;
